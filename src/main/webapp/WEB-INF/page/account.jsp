            ...
            <div class="col-md-12">
                ...
                <div class="panel" data-collapsed="0">
                    ...
                    <div id="dynamic-table_wrapper" class="dataTables_wrapper form-inline" role="grid">
                    	<div class="row-fluid">
                    		<div class="span6">
                    			<form action="${ctx}/apply/findunAccount" method="post">
                    		...
                    		<table  class="display table table-bordered table-striped dataTable" aria-describedby="dynamic-table_info" style="max_width:1000px; max_height:500px;">
  								<thead>
  									<tr>
		  								...
		  								<th align=center style="vertical-align: middle;">日均伙食费</th>
		  								...
		  								<th align=center style="vertical-align: middle;">结算</th>
		  							</tr>
		  						</thead>
  		 						<tbody>
  									<c:forEach items="${applyList}"  var="apply">
     									<tr>
     										<td align=center>${apply.userid}</td>
  											<td align=center>${apply.username}</td>
				  							<td align=center>${apply.start}</td>
				  							<td align=center>${apply.destiny}</td>
				  							<td align=center>${apply.tovehicle}</td>
				  							<td align=center>${apply.backvehicle}</td>
				  							<td align=center>
				  								<fmt:formatDate
													type="date"
				  							...
				  							<td align=center>${apply.days}</td>
				  							<td align=center>${apply.meal}</td>
				  							<td align=center>
				  								<c:choose>
				  							...
				  							<td align=center>${apply.computebudget}</td>
				  							<td align=center>${apply.budget}</td>
				  							<td align=center>
				  						<a href="${ctx}/apply/jumptoadd?applyid=${apply.applyid}&applyname=${apply.username}&userid=${apply.userid}&applypost=${apply.post}&budget=${apply.budget}">
				  						<button class="btn btn-primary" type="button">结算</button>
				  						</a>
								<!-- <a href="#">结算</a> -->
							</td>
				  		</tr>
                    ...
                    <div align="center">
            			<a href="${pageContext.request.contextPath}/apply/findunAccount">首页</a>
            			<c:if test="${pageModel.pageNo>1}"><a href="${pageContext.request.contextPath}/apply/findunAccount?pageNo=${pageModel.pageNo-1}<c:if test="${pageModel.username!=null}">&username=${pageModel.username}</c:if>">上一页</a></c:if>
            			<c:if test="${pageModel.pageNo<pageModel.totalpage}"><a href="${pageContext.request.contextPath}/apply/findunAccount?pageNo=${pageModel.pageNo+1}<c:if test="${pageModel.username!=null}">&username=${pageModel.username}</c:if>">下一页</a></c:if>
           				<c:if test="${pageModel.totalpage!=0}"><a href="${pageContext.request.contextPath}/apply/findunAccount?pageNo=${pageModel.totalpage}<c:if test="${pageModel.username!=null}">&username=${pageModel.username}</c:if>">尾页</a></c:if>
